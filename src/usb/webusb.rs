...
use web_sys::{UsbControlTransferParameters, UsbDevice, UsbInTransferResult};
...
impl UsbBackend for WebUsbBackend {
    async fn read_bulk(
        ...
        let result: UsbInTransferResult =
            JsFuture::from(self.device.transfer_in(endpoint & 0x7f, max_len as u32))
                ...
                .map_err(js_err)?
                .unchecked_into();
        Ok(transfer_result_to_vec(&result))
    }

    async fn write_bulk(&self, endpoint: u8, data: &[u8], _timeout: Duration) -> Result<()> {
    ...
    async fn read_control(
        ...
        let setup = control_setup(request, value, index)?;
        let result: UsbInTransferResult =
...
/// Build `UsbControlTransferParameters` for a Vendor/Device transfer.
///
/// The panda firmware dispatches control requests purely on `bRequest` and ignores the
/// request-type field, so the native (`rusb`) path uses the Standard type. WebUSB, however,
/// rejects Standard-type control transfers that carry non-standard `bRequest` values for
/// security ("The transfer was not allowed"), so we send them as Vendor requests instead.
fn control_setup(request: u8, value: u16, index: u16) -> Result<UsbControlTransferParameters> {
    let setup = Object::new();
    set(&setup, "requestType", JsValue::from_str("vendor"))?;
    set(&setup, "recipient", JsValue::from_str("device"))?;
...
fn transfer_result_to_vec(result: &UsbInTransferResult) -> Vec<u8> {
    match result.data() {
        Some(view) => {
